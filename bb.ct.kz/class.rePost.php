...
//		$this->conv = new ConvertCharset("utf-8", "windows-1251", false);
        ...
        '.($saleInfo['price']!=0 ? ' цена: [b][i]'.$saleInfo['price']."[/i] $[/b] *\n[size=1][i]* цена включает доставку до Бишкека[/i][/size]" : '').'
		...
		[size="1"]По вопросам приобретения звоните:
		
		Бишкек [b]+ 996 (312) 592-592[/b],
		Алматы [b]+ 8 777 8399219[/b] [i]Компания "Фаворит", г.Алматы, Тулебаева 133[/i],
		либо пишите [b][i]dmitrii@makmalauto.com[/i][/b][/size] ';
...
//        $this->text = $this->conv->Convert($this->text);  // Converting to cp1251
        ...
        $this->top_text = '[font="Georgia"][size=5][b][color="#FF0000"]"Макмал-Авто"[/color][/b][/size]
...
//        $this->top_text = $this->conv->Convert($this->top_text);  // Converting to cp1251
        $this->top_text = $this->top_text;  // Converting to cp1251

//		$this->title = $this->conv->Convert('Автомобили от компании "Макмал-Авто"');  // Converting to cp1251
		$this->title = 'Автомобили от компании "Макмал-Авто"';  // Converting to cp1251
//		$this->subtitle = $this->conv->Convert('ассортимент обновляется');  // Converting to cp1251
		$this->subtitle = 'ассортимент обновляется';  // Converting to cp1251
		
		$this->forum = 96;
		$this->topic = '349911';
		$this->mainpost = '14715295';
	...
	private function stuff() {
		...
		if($saleInfo['price']==0){
			...
			$postsubtitle = 'цена: '.$saleInfo['price'].'$';
		}
