...
class Controller_MDS_Export extends Controller_JSON {
   ...
   public function action_grid()
   {
      foreach (Kohana::config('config.items') AS $id => $values)
      {
         foreach ($values['year'] AS $year)
         {
            ...
            $data = $data_select
            ->select('mds.*', array('mds_searches.condition', 'condition'))
            ->join('mds_searches')
   ...
   public function action_stat()
   {
      ...
      $search_id = $this->request->query('search');

      $online  = $mds->online($search_id, TRUE);
      $offline = $mds->online($search_id, FALSE);
