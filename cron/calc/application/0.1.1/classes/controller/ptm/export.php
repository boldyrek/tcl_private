...
class Controller_PTM_Export extends Controller_JSON {
   ...
   public function action_grid()
   {
      foreach (Kohana::config('config.items') AS $id => $values)
      {
         ...
         {
            $query = Jelly::select('ptm')
            ->select(array(DB::expr('COUNT(*)'), 'total'), 'date_added')
            ->where('parent_id', '=', $id)
            ->and_where('year', '=', $year)
            ->order_by('id', 'DESC')
            ->limit(1)
            ...
            if ($query->loaded())
            {
               $total = (int) $query->get('total');

               $this->_body[$id][$year] = array(
                  'total' => $total,
                  'last_update' => ($total ? date(parent::DATE_FORMAT, $query->get('date_added')) : '-')
               );
            }
         }
      }
   ...
   public function action_chart()
   {
      ...
      $this->_body = $this->_data($search->as_array(), true);
   }
    ...
    * @return array
    */
   protected function _data(array $search, $include_data = FALSE)
   {
      ...
      $output = array(
         ...
         'exception' => (string) $search['exception'],
         'total' => $total
      );
