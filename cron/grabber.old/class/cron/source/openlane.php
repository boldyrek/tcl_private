...
class Cron_Source_Openlane extends Cron implements Cron_Interface {
   ...
   public function execute($source_id)
   {
      ...
      foreach ($this->_config->search->types->toArray() AS $id)
      {
         ...
         if (! (bool) preg_match('/No results found/i', $response))
         {
            if ((bool) preg_match('#<b>\s+\d+ to \d+ of (\d+)\s+</b>#isU', $response, $matches))
   ...
   public function parse($content, array $filters = NULL)
   {
      ...
      $pattern  = '#<tr class="(?:even|odd)">';
      $pattern .= '.*';
      $pattern .= '<div class="ymms">.*';
      $pattern .= '<a\s+.*href="(.+)">(.+)</a>';  // link, name
      $pattern .= '.*</div>.*';
      $pattern .= '<i>Ext: (.*)</i>'; // outside
      $pattern .= '.*';
      $pattern .= '<i>Int: (.*)</i>'; // inside
      $pattern .= '.*';
      $pattern .= '([A-Z0-9]{17})'; // vin
      $pattern .= '.*';
      ...
      try {
         ...
         foreach ($matches AS $item)
         {
            ...
            $vincode = strtoupper(Arr::get($item, 5));
            $millage = (int) str_replace(',', '', Arr::get($item, 6));
            $price = (int) str_replace(',', '', Arr::get($item, 8));
            $outside = Arr::get($item, 3, '');
            $inside = Arr::get($item, 4, '');

            $date_auction = preg_replace('/(\d+)h (\d+)m/', '$1 hours $2 minutes', Arr::get($item, 7));
