...
class userStakes extends Proto {
	...
	function addStake() {
		...
		if(!$this->error) {
			...
			$request = "
			INSERT INTO `stakes_list` (`id`,`user`,`status`,`created`,`auction`,`lot`,`sum`,`date`,`desc`)
			VALUES (
			LAST_INSERT_ID(),
			'".$_SESSION['user_id']."',
			'0',
	...
	function getData() {
		...
		SELECT stakes_list.*, stakes_auc.name as auc 
		FROM `stakes_list` 
		LEFT JOIN `stakes_auc` 
		ON (stakes_auc.id = stakes_list.auction)
		WHERE stakes_list.user = '".$_SESSION['user_id']."' 
		ORDER BY stakes_list.id DESC");
	}
