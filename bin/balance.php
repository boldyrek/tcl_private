...
function updateBalance($id,$dealer)
{
	if($dealer==0) {
		...
		$payments = mysql_fetch_array(mysql_query(
				"SELECT SUM(`ccl_".ACCOUNT_SUFFIX."accounting`.amount) as total
					...
					WHERE `ccl_".ACCOUNT_SUFFIX."cars`.buyer = '".$id."' AND `ccl_".ACCOUNT_SUFFIX."accounting`.type = '1'
						AND `ccl_".ACCOUNT_SUFFIX."accounting`.status = '1'
				ORDER BY `date` DESC"));
	...
	elseif($dealer==1) {
		...
		$mypayments = mysql_fetch_array(mysql_query("SELECT SUM(amount) as total FROM `ccl_".ACCOUNT_SUFFIX."accounting` WHERE `client` = '".intval($id)."' and `status` = '1' and type = '1'"));
		...
		WHERE ccl_".ACCOUNT_SUFFIX."customers.mydealer = '".intval($id)."' and ccl_".ACCOUNT_SUFFIX."accounting.status = '1' and ccl_".ACCOUNT_SUFFIX."accounting.type = '1'");
...
function updateDealerBalance($id, $isdealer) {
	if($isdealer == 1) {
		$mypays = mysql_fetch_array(mysql_query("SELECT SUM(amount) AS total FROM `ccl_".ACCOUNT_SUFFIX."accounting` WHERE `client` = '".$id."' and type = '1'"));
	}
	elseif($isdealer  == 0) {
...
{
	$cars = mysql_fetch_array(mysql_query("SELECT SUM(total) as total FROM `ccl_".ACCOUNT_SUFFIX."cars` WHERE `transporter`='".mysql_real_escape_string($id)."'"));
