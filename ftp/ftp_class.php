...
//echo "</pre>";
    ...
    function rmdir_all($pathname){
        $this->chdir($pathname);
        $ar_files = $this->rawlist();
        if(is_array($ar_files)){
           for($i=0;$i<count($ar_files);$i++){
               $dfl=$this->parselisting($ar_files[$i]);
               if($dfl['type']=="d" && $dfl['name']!='.' && $dfl['name']!='..') {
                  $this->rmdir_all($pathname."/".$dfl['name']);
               }elseif($dfl['type']=="f"){
                  $this->delete($pathname."/".$dfl['name']);
               }
           }
        }
        if($this->rmdir($pathname)){
           return true;
        }else{
           return false;
        }
...
// <!-- --------------------------------------------------------------------------------------- -->
// Gйnиre une erreur pour traitement externe а la classe
