...
class curl
{
	...
	function exec()
	{
		...
		if ($this->getOption(CURLOPT_HEADER))
		{
			...
			return ($this->m_header['Content-Encoding'][0]=='gzip'? gzdecode($theArray[1]) : $theArray[1]);
		}
		else
		{
			return ($this->m_header['Content-Encoding'][0]=='gzip'? gzdecode($theReturnValue) : $theReturnValue);
		}
	}

	function follow_location($result, $referer=false)
	{
		if ($this->m_status['http_code'] == 301 || $this->m_status['http_code'] == 302)
		{
			$FOLLOWLOCATION = str_replace('http://','https://', $this->m_header['Location'][0]);
			$this->setopt(CURLOPT_URL, $FOLLOWLOCATION);
			if($referer)
			{
				$this->setopt(CURLOPT_REFERER, $referer);
				$this->setopt(CURLOPT_POST, false);
				//echo "\r\n<pre>Location: ".$FOLLOWLOCATION."\r\nReferer: ".$referer."\r\n</pre>";
			}
			$result_tmp = $this->exec();
