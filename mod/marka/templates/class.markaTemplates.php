<?php
class MarkaTemplates{
	var $root_path;
	var $fieldList;
	var $translate;
	var $mass= array(0,'');

	function MarkaTemplates($root_path)
	{
	...
	{
		$this->fieldList = array($this->translate->_('Название марки автомобиля'));

	}
	
	function top_marka_link($id=0)
	{
	...
	function getTitle()
	{
		return '<h2>'.$this->translate->_('Добавление/редактирование').' '.$this->translate->_('марки автомобиля').'</h2>';
	}
	...
	function getForm()
	{
		$text=
		"
		<form name='form1' method='post' action='{$this->root_path}?mod=marka&sw=save'>
		...
		<table width='100%' border='0' cellspacing='0' cellpadding='4'>
		  ...
		  <tr>
		    <td><a href='{$this->root_path}?mod=marka&sw=delete&id=".intval($_REQUEST['id'])."'>".$this->translate->_('Удалить')."</a></td>
