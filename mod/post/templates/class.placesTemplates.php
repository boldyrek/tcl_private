<?php
class PlacesTemplates{
	var $root_path;
	var $fieldList;
	var $translate;
	var $mass= array(0,'');

	function PlacesTemplates($root_path)
	{
	...
	function top_places_link($id=0)
	{
		...
		<tr><td><a href="'.$this->root_path.'?mod=cars&sw=">'.$this->translate->_('Список автомобилей').'</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<a href="'.$this->root_path.'?mod=places">'.$this->translate->_('Список мест').'</a> |
				<a href="'.$this->root_path.'?mod=places&sw=form">'.$this->translate->_('Добавить').'</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="'.$this->root_path.'?mod=ports">'.$this->translate->_('Список портов').'</a> |
				<a href="'.$this->root_path.'?mod=ports&sw=form">'.$this->translate->_('Добавить').'</a></td>
	...
	function getTitle()
	{
		return '<h2>'.$this->translate->_('Добавление/редактирование').' '.$this->translate->_('места').'</h2>';
	}
	...
	function getForm()
	{
		$text=
		"
		<form name='form1' method='post' action='{$this->root_path}?mod=places&sw=save'>
		...
		<table width='100%' border='0' cellspacing='0' cellpadding='4'>
		  ...
		  <tr>
		    <td><a href='{$this->root_path}?mod=places&sw=delete&id=".intval($_REQUEST['id'])."'>".$this->translate->_('Удалить')."</a></td>
