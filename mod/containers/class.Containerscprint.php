...
class ContainersEdit extends Proto {
	...
	function getContent() {
		if(isset($_GET['step2'])) {
			...
			function listener(id, first) {
			 ...
			 function checkForm() {
			 	...
			 	else alert(\''.$this->translate->_('Не выбран ни один параметр!').'\');
			 }
			 function send2print() {
			 	var forma = document.forms.saveContainers;
			...
			<a onclick="send2print();" style="text-decoration:underline; cursor:pointer;">'.$this->translate->_('распечатать выбранные контейнеры').'</a></div>
			<table width="410" class="list">
			<tr class="rowB">
				<td colspan="3">'.$this->translate->_('Можно выставить одну или несколько дат.<br>Даты выставляются <u>всем выбранным контейнерам</u>.').'</td>
			</tr>
			<tr class="rowA">
				<td>'.$this->translate->_('Отправка').'</td>
			...
			<tr class="rowB">
				<td>'.$this->translate->_('Доставлен').'</td>
			...
			<tr>
				<td colspan="3"><input type="button" id="save" value="'.$this->translate->_('СОХРАНИТЬ ИЗМЕНЕНИЯ').'" onclick="checkForm();"></td>
			</tr>
	 		</table>
		...
		require_once($_SERVER['DOCUMENT_ROOT'].$this->root_path.'bin/balance.php');	
			...
			<tr>
			<td><h4 class="report">'.$this->translate->_('Изменения сохранены!').'</h4>
			<br>
			'.$this->translate->_('Для контейнеров:').' '.$this->listContainers().'<br>
			'.$this->translate->_('установлены следующие значения').':<br>
			<br>
			'.$this->showResult().'<br></td></tr>
			</table></center>';
			
		}
		else $this->redirect($this->root_path.'?mod=containers');
	}
	...
	function drawLines($content, $step) {
		...
		if(isset($_GET['nothing'])) $out .= '<tr><td colspan="3">
		 <div class="warn">'.$this->translate->_('не выбран ни один контейнер!').'</div>
		 </td></tr>';
	
		$out .= '<tr class="title">
		 <td width="80">'.$this->translate->_('контейнер').'</td>
		 <td width="170">'.$this->translate->_('экспедитор').'</td>
		 <td width="40">'.$this->translate->_('порт').'</td>
		 <td width="30">&nbsp</td>
		 </tr>';
	...
	function showResult() {
		...
		$out.='
			<tr>
			<td align="right">'.$this->translate->_('Отправка').'</td>
			<td>'.$_POST['sent'].'</td>
			</tr>';
		if($_POST['china']!='' and $_POST['chk']['china']=='on')
			$out.='<tr>
			<td align="right">'.$this->translate->_('Китай').'</td>
			<td>'.$_POST['china'].'</td>
			</tr>';
		if($_POST['rail']!='' and $_POST['chk']['rail']=='on')
			$out.='<tr>
			<td align="right">'.$this->translate->_('Ж/Д').'</td>
			<td>'.$_POST['rail'].'</td>
			</tr>';
		if($_POST['dostuck']!='' and $_POST['chk']['dostuck']=='on')
			$out.='<tr>
			<td align="right">'.$this->translate->_('Достук').'</td>
			<td>'.$_POST['dostuck'].'</td>
			</tr>';
		if($_POST['bishkek']!='' and $_POST['chk']['bishkek']=='on')
			$out.='<tr>
			<td align="right">'.$this->translate->_('Бишкек').'</td>
			<td>'.$_POST['bishkek'].'</td>
			</tr>';
		if($_POST['arrived']=='on' and $_POST['chk']['arrived']=='on')
			$out.='<tr>
			<td align="right">'.$this->translate->_('Доставлен').'</td>
			<td>ДА</td>
			</tr>';
			$out .= '</table>';
		return $out;
	...
	function updateContainersByIds($containers) {
		foreach ($containers as $k => $v) {
			...
			OR ccl_".ACCOUNT_SUFFIX."cars.id = ccl_".ACCOUNT_SUFFIX."containers.slot4
                        OR ccl_".ACCOUNT_SUFFIX."cars.id = ccl_".ACCOUNT_SUFFIX."containers.slot5)
