...
class ReportsMain extends Proto {
	...
	function process() {
		...
		if(@mysql_num_rows($this->content)!='0' and isset($_POST['to_date'])) {
			...
			$this-> page .= $this->makeResult();
					
		}
		elseif(@mysql_num_rows($query_result)=='0' and isset($_POST['to_date'])) echo $this->translate->_('За период').' "'.($_POST['from_date']!=''?'с '.$_POST['from_date'].' ':'').'по '.$_POST['to_date'].'" '.$this->translate->_('ничего не найдено').'.';
	...
	function makeResult() {
		...
		while($i<$num) {
			...
			$out .= '<tr class="'.$class.'" onmouseover="this.className=\'rowA hovered\'" onmouseout="this.className=\''.$class.'\'" onclick="document.location=\'/?mod=cars&sw=form&car_id='.$line['id'].'\'">
