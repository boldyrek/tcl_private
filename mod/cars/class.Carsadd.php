	...
	private function Process() {
		...
		while($i<$num) {
			...
			if($_POST['frame'] == $line['frame']) {
				...
				//данные формы с параметрами автомобиля
				$_SESSION['carForm']['buyer_id'] = $_POST['buyer'];
				$_SESSION['carForm']['reciever'] = $_POST['reciever'];
				$_SESSION['carForm']['car_model'] = $_POST['car_model'];
				$_SESSION['carForm']['car_marka'] = $_POST['car_marka'];
				...
				$_SESSION['carForm']['transfered'] = $_POST['transfered'];
		...
		$request = "INSERT INTO `ccl_".ACCOUNT_SUFFIX."cars` (
					...
					'".$model."', 
					'".intval($_POST['car_marka'])."', 
					'".intval($_POST['car_model'])."', 
					...
					'".intval($_POST['other'])."',
				 	...
				 	'".$date_ready."',
				 	'".intval($_POST['type'])."',
                                        '".intval($_POST['invoice_price'])."',
                                        '".mysql_real_escape_string($_POST['invoice_number'])."',
                                        '".intval($_POST['country_id'])."'
				 	)";

		$this->mysqlQuery($request);
		
		// Сразу же добавляем машину  в список "времени доставки" !!! - checkit
		$new_id = mysql_fetch_array($this->mysqlQuery("SELECT max(id) AS id FROM `ccl_".ACCOUNT_SUFFIX."cars`"));
		$this->mysqlQuery('INSERT INTO `ccl_'.ACCOUNT_SUFFIX.'daysleft` (cid) VALUES ('.$new_id['id'].')');
