		...
		elseif($_GET['what']=='photo' and $_GET['name']!=='') {
			...
			if($photo['car']!=''){
				if($_GET['chk']==$photo['car']) {
						...
						$this->redirect(((isset($_GET['from']) && $_GET['from'] == 'client') ? '/public' : '').$this->root_path.'?mod=cars&sw=form&car_id='.$photo['car']);
				}
...
// added autocheck/carfax block
		elseif($_GET['what']=='autocheck_file' and $_GET['name']!='') {
			$table = 'autocheck';
			$item = mysql_fetch_array($this->mysqlQuery("
			SELECT *
			FROM `ccl_".ACCOUNT_SUFFIX."autocheck`
			WHERE `file` = '".mysql_real_escape_string($_GET['name'])."'
			LIMIT 1"));
