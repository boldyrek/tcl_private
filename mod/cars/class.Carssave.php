...
class CarsSave extends Proto {
   ...
   private function Process()
   {
      ...
      $model = '';
      if (intval($_POST['car_model']) != 0)
      {
         $req = "SELECT ccl_".ACCOUNT_SUFFIX."marka.name, ccl_".ACCOUNT_SUFFIX."model.name FROM ccl_".ACCOUNT_SUFFIX."marka, ccl_".ACCOUNT_SUFFIX."model WHERE ccl_".ACCOUNT_SUFFIX."model.id='".intval($_POST['car_model'])."' AND ccl_".ACCOUNT_SUFFIX."marka.id='".intval($_POST['car_marka'])."' LIMIT 1";
         $res = $this->mysqlQuery($req);
         if ($res)
         {
            list ($marka, $model) = mysql_fetch_row($res);
            $model = strtoupper(mysql_real_escape_string($_POST['year'])." $marka $model");
         }
      }

      $car_is_sold = $_POST['car_is_sold'] == 'on' ? '1' : '0';

      if (isset($_POST['notice']))
         $save_notice = ", notice = '".mysql_real_escape_string($_POST['notice'])."'";
      else
         $save_notice = '';
      $request = "UPDATE `ccl_".ACCOUNT_SUFFIX."cars` SET
         buyer          = '".intval($_POST['buyer'])."',
         reciever	= '".intval($_POST['reciever'])."',
         model	        = '".mysql_real_escape_string($model)."',
         car_marka	= '".intval($_POST['car_marka'])."',
         ...
         /* transfered 	= '".mysql_real_escape_string($_POST['transfered'])."', */
         ...
         show_mileage 	= '".intval(isset($_POST['show_mileage']))."',
         ...
         allow_carfax   = '".intval($_POST['allow_carfax'])."',
         allow_cr       = '".intval($_POST['allow_cr'])."',
         type           = '".intval($_POST['type'])."',
         sold           = '".$car_is_sold."',
         invoice_price  = '".intval($_POST['invoice_price'])."',
      ...
      if ($_POST['total'] != $_POST['lastPrice'] or $_POST['buyer'] != $_POST['last_owner'] or $_POST['prepay'] != $_POST['lastPrepay'])
      {
         if ($_POST['buyer'] == $_POST['last_owner'] and $_POST['last_owner'] != '')
         {
            if ($_POST[$isdealer] == 0)
            {
               //Вот это используем, остальное для каких-то хуй понятых дилеров
               updateBalance($_POST['buyer'], 0);
               if ($_POST[$mydealer] != 0)
                  updateBalance($_POST[$mydealer], 1);
            }
            elseif ($_POST[$isdealer] == 1)
               updateBalance($_POST['buyer'], 1);
         }
         elseif ($_POST['buyer'] != $_POST['last_owner'] and $_POST['last_owner'] != '')
         {
            ...
            if ($_POST[$lastowner_isdealer] == 1 and $_POST[$lastowner_dealer] != $_POST[$mydealer])
               updateBalance($_POST['last_owner'], 1);
            elseif ($_POST[$lastowner_isdealer] == 0)
            {
               updateBalance($_POST['last_owner'], 0);
               if ($_POST[$lastowner_dealer] != 0 and $_POST[$lastowner_dealer] != $_POST['buyer'])
                  updateBalance($_POST[$lastowner_dealer], 1);
            }
         }
         ...
         elseif ($_POST['buyer'] == 0 and $_POST['last_owner'] != '')
         {
            if ($_POST[$lastowner_isdealer] == 1)
               updateBalance($_POST['last_owner'], 1);
            elseif ($_POST[$lastowner_isdealer] == 0)
            {
               updateBalance($_POST['last_owner'], 0);
               if ($_POST[$lastowner_dealer] != 0)
                  updateBalance($_POST[$lastowner_dealer], 1);
            }
         }
      }
      ...
      if ($_POST['transporter'] != 0)
         updateSupplierBalance(intval($_POST['transporter']));
