...
class CommentTemplates{
	...
	function CommentTemplates($root_path, $car_id)
	{
	...
	function header()
	{
		return "<div class='comment'><h3>".$this->translate->_('Комментарии')."</h3>";
	}
	function footer()
	{
		return '</div>';
	...
	function showOneComment($comment_items)
	{
		...
		if($comment_items['type']=='2')	$comInfo = '[ '.$this->translate->_('скрытый').' ]&nbsp;';
		else $comInfo = '';
		$text.='
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="3" id="comment_header">
				  <tr>
				    <td style="color:#626262;">'.$comInfo.$this->translate->_('Автор').': '.$comment_items['login'].'</td>
				    <td align=right style="color:#626262;">'.$this->translate->_('Дата').': <span class="dat">'.date('j.m.Y, H:i', $comment_items['dat']).'</span></td>
				  </tr>
				</table>';
	...
	function EditButton($commentId)
	{
		...
		<a href="javascript:getText('.$commentId.');">'.$this->translate->_('Редактировать').'</a>&nbsp;&nbsp;
		<a href="javascript:HideAndDel('.$commentId.');" onClick="confirm(\''.$this->translate->_('Вы действительно хотите удалить этот комментарий?').'\')">'.$this->translate->_('Удалить').'</a>
		</div>
	...
	{
		return "<div style='background:#EFEFEF;padding:5px;color:#0066CC;font-size:12px;' ><b>".$this->translate->_('Добавить комментарий')."</b></div>";
	}
	function getEditCommentFormTop()
	{
		return "<div style='background:#EFEFEF;padding:5px;color:#0066CC;font-size:12px;'><b>".$this->translate->_('Редактировать комментарий')."</b></div>";
	}
	function getEditJs()
	{
		$text="
		<script language=\"JavaScript\">			
		 ...
		 {
		 	$.get('/?mod=cars&sw=comment&what=del', { c_id: commentId },
		 	function(data) {
		 		document.getElementById('handlers_'+commentId).innerHTML = '';
		 		document.getElementById('comment_'+commentId).innerHTML='<div class=\'notice\' style=\'padding:10px;\'>".$this->translate->_('комментарий удален')."</div>';
		 	} );
	...
	{
		$text='
		<div id="showForm" style="float:left;width:340px;">
		<form  action="'.$action.'" method="post" name="comment">
		<input type="hidden" name="car_id" id="car_id" value="'.$this->car_id.'">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="4" bgcolor="#D7DFF1" style="border:1px solid #CCCCCC;">
		    ...
		    <td><b><b>'.$this->translate->_('Тип').': </b></b><select name="comment_type" id="comment_type" style="width:100px;">
		    <option value="1" selected="selected">'.$this->translate->_('для всех').'</option>
		    </select></td>
		  </tr>':'').''; 
		
		$text.='
		  <tr>
		    <td><b>'.$this->translate->_('Текст комментария').':</b><br>
		      <textarea name="comment_text" id="comment_text" style="width:100%;height:100px;"></textarea>
			</td>
		  </tr>
		  <tr>
		    <td align="right"><input name="ok" type="submit" value="'.$this->translate->_('Cохранить').'" class="button"></td>
		  </tr>
		</table></form>
		</div>


		<div id="showForm" style="float:right;width:340px;background-color:#fcc;">';

                if ($_SESSION['user_type'] != 12)
                {
                   $text .= '<form  action="'.$action.'" method="post" name="comment">
                   <input type="hidden" name="car_id" id="car_id" value="'.$this->car_id.'">
                   <input type="hidden" name="comment_id" id="comment_id" value="">
                   <table width="100%" border="0" cellspacing="0" cellpadding="4" bgcolor="#D7DFF1" style="border:1px solid #CCCCCC; background-color: transparent;">


                       '.($_SESSION["user_type"]!=2?'<tr>
                       <td><b><b>'.$this->translate->_('Тип').': </b></b><select name="comment_type" id="comment_type" style="width:100px;">
                       <option value="2" selected="selected">'.$this->translate->_('скрытый').'</option>
                       </select></td>
                     </tr>':'').'';
	...
	function showAddCommentForm()
	{
		...
		$text.=$this->getForm($this->root_path."?mod=cars&sw=comment&what=add");
