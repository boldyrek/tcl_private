...
class Webcars extends Proto {
	...
	private function _renderForm(){
		...
		$this->view->authkey = $this->_makeKey();

		ob_start();
		require_once($_SERVER['DOCUMENT_ROOT'].'/mod/cars/templates/webcarsForm.php');
		$res = ob_get_contents();
		ob_clean();
	...
	private function _renderSentForm(){
		...
		ob_start();
		require_once($_SERVER['DOCUMENT_ROOT'].'/mod/cars/templates/webcarssentForm.php');
		$res = ob_get_contents();
		ob_clean();
	...
	private function getSentInfo(){
		$sql = "SELECT * FROM `ccl_".ACCOUNT_SUFFIX."webcars_sent` WHERE `tcl_id`='".$this->car_id."' LIMIT 1";
		$res = mysql_fetch_assoc($this->mysqlQuery($sql));
		if (!empty($res)){
			$this->car_info['sent_id'] = $res['webcars_id'];
			return true;
		}
		return false;
	}
	...
	private function getMark(){
		$sql = "SELECT * FROM `ccl_".ACCOUNT_SUFFIX."webcars_marka` WHERE `tcl_id`='".$this->car_info['car_marka']."'";
		$res = mysql_fetch_assoc($this->mysqlQuery($sql));
		if (empty($res) || !$res['webcars_id']){
			die("Не найдена марка в таблице ccl_".ACCOUNT_SUFFIX."webcars_marka");
			return ;
		}
	...
	private function getModel(){
		$sql = "SELECT * FROM `ccl_".ACCOUNT_SUFFIX."webcars_model` WHERE `tcl_id`='".$this->car_info['car_model']."'";
		$res = mysql_fetch_assoc($this->mysqlQuery($sql));
		if (empty($res) || !$res['webcars_id']){
			die("Не найдена модель в таблице ccl_".ACCOUNT_SUFFIX."webcars_marka");
			return ;
		}
