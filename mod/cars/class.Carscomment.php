...
class CarsComment extends Proto {
	...
	{
		if ($_POST['comment_text']=='') return $this->translate->_('Заполните текст комментария');
		...
		if ($this->mysqlQuery($request)) {
			$this->SendMail();
			return $this->translate->_('Комментарий сохранен');
		}
		else return $this->translate->_('Комментарий сохранить не удалось');
	}

	function getCommentList()
	{
		$arr=false;
		if($_SESSION['user_type']==1) $aces="";
		else $aces="AND type='1'";
		$request="SELECT * FROM ccl_".ACCOUNT_SUFFIX."car_comment WHERE `car_id` = '".$this->car_id."' ".$aces." ORDER BY `dat`";
	...
	function SendMail()
	{
		...
		// письмо админам
		// $this->XMail("info@makmalauto.com", $email, $subj, stripslashes($text));
                
		// письмо с комментом владельцу авто
		if($type==1) {
			if(strlen($info['email'])>6) {
				$this->XMail("info@makmalauto.com", $info['email'], $subj, stripslashes($text));
			}
		}
	}
	...
	{
		$sql="SELECT `email` from `ccl_".ACCOUNT_SUFFIX."usrs` WHERE type = 1 AND `id`!='".$_SESSION['login_id']."' AND `email`!=''";
