...
class ExpensesList extends Proto {
...
/*		if(isset($_GET['list'])) {
		...
		FROM `ccl_".ACCOUNT_SUFFIX."expenses` WHERE 1"));
		...
		$request = "
		SELECT ccl_".ACCOUNT_SUFFIX."expenses.*, ccl_".ACCOUNT_SUFFIX."cars.model, ccl_".ACCOUNT_SUFFIX."cars.frame, ccl_".ACCOUNT_SUFFIX."usrs.log_name
		FROM `ccl_".ACCOUNT_SUFFIX."expenses` 
		LEFT JOIN `ccl_".ACCOUNT_SUFFIX."cars`
		ON ccl_".ACCOUNT_SUFFIX."cars.id = ccl_".ACCOUNT_SUFFIX."expenses.car
		LEFT JOIN `ccl_".ACCOUNT_SUFFIX."usrs`
		ON ccl_".ACCOUNT_SUFFIX."usrs.id = ccl_".ACCOUNT_SUFFIX."expenses.user_added";
...
/*		switch ($_SESSION['expenses_list']) {
		...
		$cols[1] = array('name' => 'date', 		'caption' => 'Дата', 		'width' => '70');
		$cols[2] = array('name' => 'model',		'caption' => 'Автомобиль',	'width' => '230');
		$cols[3] = array('name' => 'frame', 	'caption' => 'VIN', 		'width' => '75');
		$cols[4] = array('name' => 'amount', 	'caption' => 'Сумма', 		'width' => '75');
		$cols[5] = array('name' => 'log_name',	'caption' => 'Добавил(а)',	'width' => '80');
//		$cols[6] = array('name' => 'purpose', 	'caption' => 'Назначение',	'width' => '80');
		$cols[6] = array('name' => 'comment', 	'caption' => 'Комментарий', 'width' => '230');
		$cols[7] = array('name' => 'signer', 	'caption' => 'Подтвердил(а)','width' => '80');
		$cols[8] = array('name' => 'paid',		'caption' => 'Оплачено',	'width' => '');
...
/*		$customers = $this->getCustomersList();
		...
		<tr><td>Платежи | <a href="'.$this->root_path.'?mod=expenses&sw=form&add">добавить</a></td>
		<td>&nbsp;</td>
		<td width="350" align="right"><form method="post" action="'.$this->root_path.'?mod=expenses&filter" style="margin:0px;" class="myForm">
		<table>
		<tr><td align="right">
		авто: '.$cars_list.'
		</td>
