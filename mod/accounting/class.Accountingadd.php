...
class AccountingAdd extends Proto {
    ...
    private function Process() {
        ...
        // Подсчет статуса ?!?!?!?
        if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $status = '1'; else $status = '0';

        // Выбор типа записи
        if($_POST['acctype']=='2') $acctype = '2'; else $acctype = '1';

        // размер платежа/ расхода должен быть положительным
        ...
        $request = "INSERT INTO `ccl_".ACCOUNT_SUFFIX."accounting` (`client`, `amount`, `comment`, `date`, `user_added`, `status`, `last_edited`, `$what`, `paid`, `signer`, `purpose`, `type`)
		VALUES (
		'0',
		'".$amount."', 
        ...
        if ($what=='car'){
            $sql = "SELECT * FROM `ccl_".ACCOUNT_SUFFIX."cars` WHERE id='".$_POST[$what]."'";
        ...
        */
        ...
        else $this->redirect($this->root_path.'?mod=accounting');
    }
}
?>