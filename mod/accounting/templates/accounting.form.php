...
//
//##############################
function accountingForm ($mode, $id, $content, $cars, &$purposes) {
    global $root_path;
            /*if(isset($_GET['add'])) */
    $cars_list = iconv('UTF-8', 'WINDOWS-1251', $cars);
        //	else $cars_list = '';
    ...
    //require($_SERVER['DOCUMENT_ROOT'].'/inc/AccountingPurposes.php');

    $purpose='<select name="purpose" id="purpose">';
        // onchange="puttext(\'purpose\',\'comment\');"
    ...
    if($content['last_edited']!="0000-00-00 00:00:00") $last_edited = $translate->_('последнее изменение').': <span style="background-color:#777;color:#ddd"> &nbsp;&nbsp;&nbsp;<b>'.$content['last_edited'].'</b> &nbsp;&nbsp;&nbsp;</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; '.$translate->_('создал').': <span style="background-color:#777;color:#ddd"> &nbsp;&nbsp;&nbsp;<b>'.$content['log_name'].'</b> &nbsp;&nbsp;&nbsp;</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; '.$translate->_('подтвердил').': <span style="background-color:#777;color:#ddd"> &nbsp;&nbsp;&nbsp;<b>'.$content['signer'].'</b> &nbsp;&nbsp;&nbsp;</span>';
    ...
    function switchCars(id) {
	    $.get("/?mod=accounting&sw=cars", { owner: id },
			 	function(data) {
	...
	<h3>'.($content['type']=='2' ? $translate->_('Расход') : $translate->_('Приход') ).'</h3>
	<table width="692" border="0" cellpadding="0" cellspacing="0" class="list">
	  <tr>
		<td width="80" align="right" class="title">&nbsp;</td>
		<td width="253" class="rowA title" align=right>'.$translate->_('Сумма').'</td>
	  ...
	  <tr>
		...
		<td align="right" class="title rowB">'.$translate->_('дата').'</td>
	  ...
	  <tr>
		<td align="right" class="title">'.$translate->_('Назначение платежа').'</td>
		...
		<td class="rowA title" style="text-align:left;"><label for="confirm">'.$translate->_('Оплачено?').'</label></td>
		<td>&nbsp;</td>
	  </tr>
	  <tr>
	...
	<table width="702" border="0" cellpadding="0" cellspacing="0" class="list">
	  <tr>
		<td class="title"><a href="'.$root_path.'?mod=accounting&sw=delete&id='.$id.'&client='.$content['client'].'&dealer='.$content['dealer'].'" class="delete" onclick="return confirm(\''.$translate->_('Вы действительно хотите удалить этот платеж?').'\')">'.$translate->_('удалить').'</a></td>
