...
if(Proto::exists($_GET['sw'])) {
	$file = $_SERVER['DOCUMENT_ROOT'].'/mod/accounting/class.Accounting'.$_GET['sw'].'.php';
	if(file_exists($file)) require_once($file);
	else Proto::redirect('/');
}
else require_once($_SERVER['DOCUMENT_ROOT'].'/mod/accounting/class.AccountingList.php');

switch($_GET['sw']) {
	case 'form':$page = new AccountingForm();break;
	case 'save':$page = new AccountingSave();break;
	case 'delete':$page = new AccountingDelete();break;
	case 'add':$page = new AccountingAdd();break;
	case 'cars':$page = new CarsList();break;
	case 'sign':$page = new SignAccounting();break;
		
	default:
		$page = new AccountingList();
		break;
}
