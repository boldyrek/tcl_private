<?
// Used to upload files using multiupload feature in Firefox >= 3.6, Chrome etc...
...
class multiUpload extends Proto {
	...
	function getFileList() {
		...
		if(count($list)>0) {
			foreach ($list as $k=>$v) {
				$this->file_type = $this->defineTypo($_FILES['carphotos']['name'][$k]);
				if($this->file_type=='jpg' or $this->file_type=='jpeg' or $this->file_type=='gif' or $this->file_type=='png') {
					$_FILES['file']['tmp_name'] = $_FILES['carphotos']['tmp_name'][$k];
					$_FILES['file']['name'] = $_FILES['carphotos']['name'][$k];
					
					$this->putOneFile();
				}
			}
		}
		$this->redirect("/?mod=".($this->owner_type=='car'?'cars':$this->owner_type)."&sw=form&".$this->owner_type."_id=".$this->owner_id);
	}
