...
if(Proto::exists($_GET['sw'])) {
	$file = $_SERVER['DOCUMENT_ROOT'].'/mod/payments/class.Payments'.$_GET['sw'].'.php';
	if(file_exists($file)) require_once($file);
	else Proto::redirect('/');
}
else require_once($_SERVER['DOCUMENT_ROOT'].'/mod/payments/class.PaymentsList.php');

switch($_GET['sw']) {
	case 'form':$page = new PaymentsForm();break;
	case 'save':$page = new PaymentsSave();break;
	case 'delete':$page = new PaymentsDelete();break;
	case 'add':$page = new PaymentsAdd();break;
	case 'cars':$page = new CarsList();break;
		
	default:
		$page = new PaymentsList();
		break;
}
