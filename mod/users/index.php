<?
/*
	Индексная страница раздела менеджмента пользователей.
...
if(Proto::exists($_GET['sw'])) {
	$file = $_SERVER['DOCUMENT_ROOT'].'/mod/users/class.Users'.$_GET['sw'].'.php';
	if(file_exists($file)) require_once($file);
	else Proto::redirect('/');
}
else require_once($_SERVER['DOCUMENT_ROOT'].'/mod/users/class.UsersList.php');

switch($_GET['sw']) {
	case 'form':$page = new UsersForm();break;
	case 'save':$page = new UsersSave();break;
	case 'delete':$page = new UsersDelete();break;
	case 'add':$page = new UsersAdd();break;
