<?
class PlacesSave extends Proto {
	...
	function Process()
	{
		...
		if ($error=='')
		{
			if ($Id==0)
			{
				$request="INSERT INTO ccl_".ACCOUNT_SUFFIX."places SET
			name='".htmlspecialchars($_POST['name'])."'";
			}
			else
			{
				$request="UPDATE ccl_".ACCOUNT_SUFFIX."places SET
			...
			header('location:'.$this->root_path.'?mod=places&sw=');exit;
		}
		else
		{
			$_SESSION['error']=$error;
			header('location:'.$this->root_path.'?mod=places&sw=form&id='.$Id);exit;
		}
	}
}
?>