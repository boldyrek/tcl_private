...
if(Proto::exists($_GET['sw'])) {
	$file = $_SERVER['DOCUMENT_ROOT'].'/mod/expeditors/class.Expeditors'.$_GET['sw'].'.php';
	if(file_exists($file)) require_once($file);
	else Proto::redirect('/');
}
else require_once($_SERVER['DOCUMENT_ROOT'].'/mod/expeditors/class.ExpeditorsList.php');

switch($_GET['sw']) {
	case 'form':$page = new ExpeditorsForm();break;
	case 'save':$page = new ExpeditorsSave();break;
	case 'delete':$page = new ExpeditorsDelete();break;
	case 'add':$page = new ExpeditorsAdd();break;
		
	default:
		$page = new ExpeditorsList();
		break;
}
