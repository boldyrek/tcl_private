...
class PortsList extends Proto {
	...
	function getContent() {
		...
		$total_items = mysql_fetch_array($this->mysqlQuery(" SELECT COUNT(id) AS total  FROM `ccl_".ACCOUNT_SUFFIX."ports` "));
		if($total_items['total']>$this->per_page)
		{
			$pages = $this->pageBrowse($_GET['page'], $_GET['mod'], $total_items['total']);
		}
		$request = "SELECT * FROM ccl_".ACCOUNT_SUFFIX."ports";

		//задаем параметры таблицы
		$cols[1] = array('name' => 'name', 	'caption' => $this->view->fieldList[0]);

		$this->page .= $this->view->top_ports_link();
		$this->page .= $this->buildList($request, $cols, 'ports', 'ports', 'id', $pages['qlimit']);
