<?php
class PortsTemplates{
	var $root_path;
	var $fieldList;
	var $translate;
	var $mass= array(0,'');

	function PortsTemplates($root_path)
	{
	...
	function getTitle()
	{
		return '<h2>'.$this->translate->_('Добавление/редактирование').' '.$this->translate->_('ПОРТА').'</h2>';
	}
	...
	function getForm()
	{
		$text=
		"
		<form name='form1' method='post' action='{$this->root_path}?mod=ports&sw=save'>
		...
		<table width='100%' border='0' cellspacing='0' cellpadding='4'>
		  ...
		  <tr>
		    <td><a href='{$this->root_path}?mod=ports&sw=delete&id=".intval($_REQUEST['id'])."'>".$this->translate->_('Удалить')."</a></td>
