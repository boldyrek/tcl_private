...
function supForm($mode, $content, $cars) {
	global $root_path;
...
<div class="cont_car">
	<h3>'.$translate->_('Транспортник').'</h3>
	<table width="692" border="0" cellpadding="0" cellspacing="0" class="list">
	  ...
	  <tr>
		...
		<td class="rowA rowB title">'.str_replace('.',',',$content['balance']).'</td>
	  ...
	  <tr>
		<td class="rowA rowB title" align="right">&nbsp;</td>
	...
	<table width="702" border="0" cellpadding="0" cellspacing="0" class="list">
	  <tr>
        ...
        {
           $out .= '<a href="'.$root_path.'?mod=transporters&sw=delete&id='.intval($_GET['sup_id']).'" class="delete" onclick="return confirm(\''.$translate->_('Вы действительно хотите удалить этого поставщика?').'\')">'.$translate->_('удалить').'</a>';
        }
	...
	// услуги и платежи поставщика
	//проверяем есть ли у поставщика машины
	
	if(@mysql_num_rows($cars) != '0')
	{
		$class = "rowA";
		...
		while ($i<=$num)
		{
			...
			$cars_list .= '<tr class="'.$class.'" onmouseover="this.className=\'rowA hovered\'" onmouseout="this.className=\''.$class.'\'">
				<td>'.$line['date'].'</td>
				<td align="center"><b>'.str_replace('.',',',$line['costtp']).'</b></td>
				<td><i>'.$line['model'].'</i> - <span style="color:navy">'.$line['frame'].'</span>&nbsp;</td>
				<td onclick="document.location=\''.$root_path.'?mod=cars&sw=form&car_id='.$line['id'].'\'"><img src="'.$root_path.'img/ccl/goto.gif"></td>
				</tr>';
				$i++;
	...
	if(!isset($_GET['add']))
	{
	$supplier_payments = mysql_query("SELECT * from `ccl_".ACCOUNT_SUFFIX."sup_pay` WHERE `to`='".$content['id']."' ORDER BY date DESC");

	$sup_services = mysql_query("SELECT * from `ccl_".ACCOUNT_SUFFIX."sup_serv` WHERE `from`='".$content['id']."' ORDER BY date DESC");

	
	$out .= '<div style="background-color:#eee; width:692px;padding:4px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
		  <tr class="title">
			<td width="50%" style="border-right:1px solid #ccc;"><b>'.$translate->_('Услуги поставщика').'</b><img src="img/ccl/bl_plus.gif" align="absmiddle" hspace=5 border="0" style="cursor:hand; cursor:pointer;" alt="'.$translate->_('Добавить услугу').'" onclick="document.getElementById(\'addService\').style.display=\'\';">
			...
			<form name="add_service" action="'.$root_path.'?mod=transporters&sw=add&add=supservice&id='.$content['id'].'" method="post"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
			  <tr>
			...
			<td width="50%"><b>'.$translate->_('Платежи поставщику').'</b>
			<img src="img/ccl/bl_plus.gif" align="absmiddle" hspace=5 border="0" style="cursor:hand; cursor:pointer;" alt="Добавить услугу" onclick="document.getElementById(\'addPayment\').style.display=\'\';">
			...
			<form name="add_payment" action="'.$root_path.'?mod=transporters&sw=add&add=suppayment&id='.$content['id'].'" method="post"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
			  <tr>
				<td class="title">'.$translate->_('дата').'</td>
			  </tr>
			  <tr>
		  ...
		  <tr>
			<td width="50%" style="border-right:1px solid #ccc;" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  <tr class="title">
				...
				<td width="60" align="center">'.$translate->_('сумма').'</td>
				<td>'.$translate->_('описание').'</td>
				<td width="15">&nbsp;</td>
			  </tr>
			'.$cars_list.makeList($sup_services, 'supservice', $_GET['sup_id']).'
			</table></td>
			...
			'.makeList($supplier_payments, 'suppayment', $_GET['sup_id']).'
			</table></td>
		  </tr>
		</table></div>';
	}
...
function makeList($list, $who, $type) {
	...
	while ($i<=$num)
	{
		...
		$print .= '<tr class="'.$class.'" onmouseover="this.className=\'rowA hovered\'" onmouseout="this.className=\''.$class.'\'" >
			...
			<td width="40" align="center"><b>'.str_replace('.',',',$line['sum']).'</b></td>
			<td style="color:#333">'.$line['comment'].'&nbsp;</td>
			<td><a href="'.$root_path.'?mod=transporters&sw=delete&what='.$who.'&id='.$line['id'].'&parent='.$type.'"><img src="img/ccl/del_img.gif" onclick="return confirm(\''.$translate->_('Вы действительно хотите удалить эту запись?').'\')" border="0"></a></td>
			</tr>';
			$i++;
			if ($class=="rowA") $class="rowA rowB"; else $class="rowA";
	}
