...
class Core extends Proto {
   ...
   protected function cronInProgress()
   {
      /*
        $query = mysql_query('SELECT COUNT(id) AS `total` FROM `ccl_spider_statuses` WHERE `locked` = 1');
   ...
   public function getList()
   {
      ...
      {
         $this->page .= '<div class="success message loader">'.$this->translate->_('Подождите, идет процесс импорта данных...').'</div>';
      }
      else
      {
         ...
         {
            $query = mysql_query("SELECT COUNT(id) AS `total` FROM `ccl_spider_cars` WHERE `target_id` = '{$id}'") or die(mysql_error());
            ...
            if ($total_cars > 0)
            {
               $query = mysql_query("SELECT v.vincode, v.date_added FROM `ccl_spider_vins` v, `ccl_spider_cars` c WHERE v.vincode = c.vincode AND v.target_id = '{$id}' ORDER BY v.date_added DESC LIMIT 1");
            ...
            $query = mysql_query("SELECT `date_last_updated` AS `date` FROM `ccl_spider_statuses` WHERE `target_id` = '{$id}' ORDER BY `id` DESC LIMIT 1");
      ...
      <script type="text/javascript">
         ...
         timeoutObserver = setInterval(timeout, interval*'.count($this->cfg['root']['targets']).'*60*1000);
         // timeoutObserver = setInterval(timeout, 5000);
   ...
   public function getGrid()
   {
      ...
      else
      {
         ...
         $query = mysql_query('SELECT COUNT(*) AS `total` FROM `ccl_spider_cars` WHERE `target_id` = '.$id);
         $row = mysql_fetch_array($query, MYSQL_ASSOC);
         $total = (int) $row['total'];
         $pages = array();
         ...
         if ($total > 0)
         {
            ...
            $sql = 'SELECT * FROM `ccl_spider_cars` WHERE `target_id` = '.$id.' ORDER BY `'.(empty($_GET['sortby']) ? 'date_added' : $_GET['sortby']).'` '.(empty($_GET['order']) ? 'DESC' : $_GET['order']).' '.$pages['qlimit'];
            ...
            while ($row = mysql_fetch_object($query))
            {
               ...
               $this->page .= '
                  ...
                  <td class="sm">'.($row->price != 0 ? $row->price : '-').'</td>
                  <td class="sm">'.(strtotime($row->date_auction) ? date('d.m.Y', strtotime($row->date_auction)) : '-').'</td>
                  <td class="sm">'.$exterior.'</td>
                  <td class="sm">'.$interior.'</td>
                  <td class="sm">'.(! empty($row->date_made) ? $row->date_made : '-').'</td>
                  <td class="sm">'.(! empty($row->options) ? '<span class="options-span" id="options-'.$row->id.'">Опции</span>' : '-').'</td>
   ...
   protected function getStyles()
   {
      ...
      <style type="text/css">
         ...
         .options-span {color:#0000ff; border-bottom:1px dotted #0000ff;}
      </style>
      ';
   }
