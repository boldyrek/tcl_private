...
class StuffComment extends Proto {
	var $stuff_id=0;
	var $view;
	var $carOwnerId;
	...
	function _LoadView()
	{
		$this->view=new CommentTemplates($this->root_path, $this->stuff_id);
	}

	function getContent()
	{
		$this->stuff_id=intval($_REQUEST['stuff_id']);
		...
		if (isset($_GET['what']))
		{
			...
			header("location:".$this->root_path."?mod=stuff&sw=form&stuff_id=".$this->stuff_id);exit;
		}
	...
	function AddEditComment()
	{
		...
		if($this->stuff_id!=0) {
			...
			if ($commentId==0)
			{
				$request="INSERT INTO ccl_".ACCOUNT_SUFFIX."stuff_comment SET user_id='".$_SESSION['login_id']."', stuff_id='".$this->stuff_id."',
			...
			else
			{
				...
				{
				$request="UPDATE ccl_".ACCOUNT_SUFFIX."stuff_comment SET type='".$type."',
	...
	{
		$arr=false;
		$request="SELECT * FROM ccl_".ACCOUNT_SUFFIX."stuff_comment WHERE `stuff_id` = '".$this->stuff_id."'".($_SESSION['user_type']==2?"AND type='1'":"")."ORDER BY `dat`";
	...
	function SendMail()
	{
		...
		// письмо админам
		$this->XMail("dmitrii@makmalauto.com", $email, $subj, stripslashes($text));
		// письмо с комментом владельцу авто
		if(intval($_POST['comment_type'])==1) {
			if(strlen($info['email'])>6) {
				$this->XMail("dmitrii@makmalauto.com", $info['email'], $subj, stripslashes($text));
			}
		}
	}
	...
	{
		$sql="select * from ccl_".ACCOUNT_SUFFIX."stuff_comment where `id`='$id'";
	...
	function GetCarInfo()
	{
		...
		on (t2.id = t1.buyer)
		WHERE t1.id='".$this->stuff_id."' LIMIT 1";
		if ($res=$this->mysqlQuery($sql))
		{
	...
	function delComment() {
		...
		if($id!='' and $id!=0) {
			if($this->_CheckOwner($id)) {
				$sql = "DELETE FROM `ccl_".ACCOUNT_SUFFIX."stuff_comment` WHERE `id` = '".$id."'";
				$this->mysqlQuery($sql);
			}
