...
class CarsList extends Proto {
	...
	function getContent() {
		...
		/** 20080321 BW **/
		if(isset($_GET['viewonly']) and intval($_GET['viewonly'])>0 and intval($_GET['viewonly'])<4) $_SESSION['viewonly'] = intval($_GET['viewonly']);
...
/** /20080321 **/
		...
		$customers_list = buildSelect($customers, 'searchClient', $_SESSION['client_filter'], ' - - - ', '5');
		
		//список мест для поиска
		$places_list = buildSelectArray($this->place, 'place1', $_SESSION['place1'], ' - - - ', '6');
		$places_list3 = buildSelect($this->getPlacesList(), 'place3', $_SESSION['place3'], ' - - - ', '7');
		...
		<tr>
			<td width="290" nowrap>Автомобили | <a href="'.$add_link.'">добавить</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.$this->root_path.'?mod=places">Список мест</a> &nbsp;&nbsp;&nbsp;&nbsp;<a href="'.$this->root_path.'?mod=ports">Список портов</a></td>
		...
		<td align="right"><table cellspacing="0" cellpadding="0" class="title noborder">
		  <tr>
		    <td nowrap>место нахождения авто</td>
		    <td width="245">'.$places_list.'</td>
		    <td width="80" nowrap>место назначения</td>
		    <td width="325">'.$places_list3.'</td>
		  </tr>
		  <tr>
		    <td nowrap>&nbsp;</td>
		    <td width="150">&nbsp;</td>
		    <td width="80" nowrap>и владельцу </td>
		...
		<td style="text-align:right;">
			<input name="submit" type="submit" style="width:90%;" value="найти">
		</table>
		</form>';
		...
		*/
		/** **/
		$request = "
		SELECT `ccl_".ACCOUNT_SUFFIX."car_comment`.`car_id`,`ccl_".ACCOUNT_SUFFIX."usrs`.`type` FROM `ccl_".ACCOUNT_SUFFIX."car_comment`, `ccl_".ACCOUNT_SUFFIX."usrs` 
		WHERE `ccl_".ACCOUNT_SUFFIX."car_comment`.`dat` IN (select max(`dat`) FROM `ccl_".ACCOUNT_SUFFIX."car_comment` group by `car_id`) 
		and `ccl_".ACCOUNT_SUFFIX."car_comment`.`user_id`=`ccl_".ACCOUNT_SUFFIX."usrs`.`id` and `ccl_".ACCOUNT_SUFFIX."usrs`.`type`=2
		";
		...
		$this->page .= '<div class="location" style="width:970px">'.$search->parser().'
			...
			'.$this->sorterTD('cars', 'buy_date', 'дата покупки', '105').'
			'.$this->sorterTD('cars', 'frame', 'вин код', '75').'
			'.$this->sorterTD('cars', 'cust_name', 'владелец', '250').'
			'.$this->sorterTD('cars', 'model', 'модель', '170').'
			'.$this->sorterTD('cars', 'number', 'контейнер', '80').'
			'.$this->sorterTD('cars', 'sup_name', 'транспортник', '90').'
			'.$this->sorterTD('cars', 'port_name', 'порт', '').'
			'.$this->sorterTD('cars', 'place_id1', 'местонахождение авто', '120').'
			'.$this->sorterTD('cars', 'place_id2', 'местонахождение тайтла', '90').'
			'.$this->sorterTD('cars', 'auction_name', 'Аукцион', '').'
			<td></td>
		  </tr>';
		...
		while ($i<=$num)
		{
			...
			$j=1;
			if($line['name']=='') $line['name'] = '- = не выбран = -';
			...
			$this->page .= '<tr class="'.$class.'" onmouseover="this.className=\'rowA hovered\'" onmouseout="this.className=\''.$class.'\'"  onclick="document.location=\''.$item_link.$line['id'].'\'">
				...
				<td class="sm" style="font-family:monospace;">'.substr($line['frame'],0,2).'.'.substr($line['frame'],(strlen($line['frame'])-8),8).'&nbsp;</td>
				<td class="sm">'.(cleanContent($line['cust_name'])==''?'&nbsp;':cleanContent($line['cust_name'])).'</td>
				<td class="sm">'.($line['model']==''?'&nbsp;':$line['model']).'</td>
				...
				<td class="sm">'.$this->place[$line['place_id2']].'&nbsp;</td>
	...
	function switchCarsviewList() {
		...
		$out = '
		<div style="width:100px;" class="tabs'.($sw==1?' tsel':'').'" onclick="document.location=\''.$this->root_path.'?mod=cars&viewonly=1\'">новые</div>
		<div style="width:100px;" class="tabs'.($sw==2?' tsel':'').'" onclick="document.location=\''.$this->root_path.'?mod=cars&viewonly=2\'">в пути</div>
		<div style="width:100px;" class="tabs'.($sw==3?' tsel':'').'" onclick="document.location=\''.$this->root_path.'?mod=cars&viewonly=3\'">архив</div>';
		return $out;
	}
