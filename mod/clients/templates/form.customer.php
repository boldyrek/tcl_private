...
function customerForm($mode, $content, $dealers, $id, $total, $ontheway, $paid, $prepay) {
	...
	if($content['balance']<0) $advance=" (".$translate->_('аванс').")";
	...
	if($content['scan']!='')
	{
		...
		<a href="'.$root_path.'?mod=clients&sw=delete&what=scan&name='.$content['scan'].'" class="delete" onclick="return confirm(\''.$translate->_('Вы действительно хотите удалить прикрепленный файл?').'\')">'.$translate->_('удалить').'</a></div>';
	}
	else {
	...
	<br><input type="submit" value="'.$translate->_('Загрузить').'" id="save"></form>';
	}
	
	//список всех дилеров Дилеры в нашей системе больше не используутся, надо бы их удалить вообще
//	$myDealers = buildSelect($dealers, 'mydealer', $content['mydealer'], $translate->_('не выбран'), '10');
	...
	<div style="float:right;font-size:11px;"><a href="'.$root_path.'?mod=client_cars&client='.$content['id'].'">'.$translate->_('автомобили клиента').'</a></div>
	<h3>'.$translate->_('Клиент').'</h3>
	<table width="792" border="0" cellpadding="0" cellspacing="0" class="list">
	  <tr>
		<td width="113" align="right" class="title">'.$translate->_('ФИО').'</td>
		<td width="253" class="rowA title"><input type="text" name="name" value="'.cleanContent($content['name']).'" tabindex="1" /></td>
		';
		if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $out .= '<td align="right" class="title" width="150">'.$translate->_('доставлено на сумму').':</td>
	  ...
	  <tr>
		<td align="right" class="title rowB">'.$translate->_('адрес').'</td>
		<td class="rowA rowB title"><input type="text" name="address" value="'.$content['address'].'" tabindex="2"/></td>
		';
		if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $out .= '<td align="right" class="title rowB">'.$translate->_('предоплата<br>за машины в пути').':</td>
	  ...
	  <tr>
		...
		';
		if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $out .= '<td align="right" class="title">'.$translate->_('платежи').'</td>
	  ...
	  <tr>
		...
		if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $out .= '<td align="right" class="title rowB">'.$translate->_('к оплате').'</td>
	  ...
	  <tr>
		<td align="right" class="title">'.$translate->_('контакты').'</td>
		<td class="rowA title"><input type="text" name="contacts" value="'.$content['contacts'].'" tabindex="5" /></td>';
		if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $out .= '<td align="right" class="title">'.$translate->_('машины в пути').'</td>
	  ...
	  <tr>
		<td align="right" class="title rowB">'.$translate->_('паспорт').'</td>
		<td class="rowA rowB title"><input type="text" name="passport" value="'.$content['passport'].'" tabindex="6" /></td>';
		if($_SESSION['user_type']=='1' or $_SESSION['user_type']=='7') $out .= '<td align="right" class="title rowB">'.$translate->_('всего машин заказано').'</td>
 ...
 <!-- <tr>
    ...
    <td style="font-size:11px;border:0px;"><label for="dealer" style="cursor:hand; cursor:pointer;">'.$translate->_('дилер').'</label></td>
  </tr>  -->
</table></td>
	  </tr>
 <!--	  <tr>
		...
		<td class="title rowB">&nbsp;</td>
		<td class="title rowB" align="right">'.$translate->_('дилер этого клиента').':</td>
		<td class="rowA rowB title">'.$myDealers.'&nbsp;</td>
	  </tr> -->
	  <tr>
	    <td class="title" align="right">'.$translate->_('логин').':</td>
	    <td class="rowA title">';
		if($id!='') $out .= '<input type="text" value="'.ClientID($id).'" name="login" tabindex="11">';
		if(isset($_GET['error'])) $out .= '<br><span style="font-size:11px; color:#f00;">'.$translate->_('логин').' <b>'.$_GET['error'].'</b> '.$translate->_('уже существует').'!</span>';
		$out .='</td>
	    <td class="title">&nbsp;</td>
	    <td class="rowA title">&nbsp;</td>
      </tr>
	  <tr>
	    <td class="title rowB" align="right" valign="top">'.$translate->_('пароль').':</td>
	    <td class="rowA rowB title">';
		if($id!='') $out .= '<input type="text" name="password" tabindex="12">
		<br><span style="font-size:10px; color:#777;">'.$translate->_('пароль можно только сменить').'!</span>';
		$out.='</td>
	    <td class="title rowB" align="right">'.$translate->_('контракт').'</td>
	...
	<table width="802" border="0" cellpadding="0" cellspacing="0" class="list">
	  <tr>
		<td class="title">'.(isset($_GET['add'])?'':'<a href="'.$root_path.'?mod=clients&sw=delete&id='.$id.(isset($_GET['hidemenu'])?'&hidemenu':'').'" class="delete" onclick="return confirm(\''.$translate->_('Вы действительно хотите удалить этого клиента?').'\')">'.$translate->_('удалить').'</a>').'</td>
		<td width="214" align="right" class="title"><input type="submit" name="Submit" value="'.$translate->_('Сохранить').'" id="save" tabindex="13" /></td>
	...
	<table width="230" class="list"><tr>
	    <tr><td class="title rowB">'.$translate->_('копия паспорта').':</td></tr>
...
function contractsList($data) {
	...
	if($num>0) {
		...
		$out = $translate->_('нет договоров').', <a href="'.$root_path.'?mod=contracts&sw=add&client='.intval($_GET['customer_id']).'">'.$translate->_('создать').'</a>';
	}
	return ltrim($out,', ');
}
